...
class My_Environment():
    ...
    #This function checks for collisions with vertical walls if any and returns the point of collision if so applies.
    ...
    def checkObsVertical(self, x1, y1, x2, y2, w1, z1, w2, z2):
        ...
        if abs(poi_x - x2) + abs(poi_x - x1) == abs(x2 - x1) and abs(poi_y - y2) + abs(poi_y - y1) == abs(y2 - y1) and abs(poi_y - z2) + abs(poi_y - z1) == abs(z2 - z1):
            return 1, [poi_x, poi_y]
        else:
            return 0, [x2, y2]

    #This Function checks for collisions with horizontal walls if any. As of now I'm keeping this unused as I haven't
    #programmed any horizontal walls into the matrix yet.
    ...
    def checkObsHorizontal(self, x1, y1, x2, y2, w1, z1, w2, z2):
        if (x1 == x2):
            ...
            if abs(poi_x - x2) + abs(poi_x - x1) == abs(x2 - x1) and abs(poi_y - y2) + abs(poi_y - y1) == abs(
                    y2 - y1) and abs(poi_x - w1) + abs(poi_x - w2) == abs(w2 - w1):
                return 1, [poi_x, poi_y]
            else:
                return 0, [x2, y2]
        ...
        if abs(poi_x - x2) + abs(poi_x - x1) == abs(x2 - x1) and abs(poi_y - y2) + abs(poi_y - y1) == abs(y2 - y1) and \
	...
	theFile = open(self.fileName, "a");
        ...
        if(A == 0):
            ...
            #Potential new position is calculated using basic trigonometry and gaussian noise is added to the result
            ...
            #Checking if the new positions are within bounds. If not then appropriate new postion is calculated and assigned.
            ...
            #If the new position is within bounds then check if there are any collisions on the way
            ...
            if(outOfBounds == 0):
                for i in range(5):
                    ...
                    # poi receives the x and y coordinates of the point of collision with wall if there is one.

                    status, poi = self.checkObsVertical(x1, y1, x2, y2, w1, z1, w2, z2)
                    if(status == 1): break

                #If there is a collision then loop with break at index i and status will be set to 1
                #We can now use the coordinates of the wall and the point of collision to calculate the appropriate new postion
		theFile.write("%.2f, " % self.pos_x)
		theFile.write("\t");
		theFile.write("%.2f, " % self.pos_y)
		theFile.write("\t");		    
	    ...
	    if(new_pos_x > 20):
            ...
            if(A == 1):
                ...
                if(self.orient < 0): self.orient += 2*np.pi
                final_x = self.pos_x
                final_y = self.pos_y
            else:
                if(A == 2):
                    self.orient = self.orient + np.pi/6
                    if (self.orient > 2 * np.pi): self.orient += -2 * np.pi
                    final_x = self.pos_x
                    final_y = self.pos_y
